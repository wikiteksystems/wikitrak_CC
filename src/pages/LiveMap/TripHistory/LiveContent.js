...
import { Box, Card,Dialog,DialogTitle, DialogContent,Modal,Popover  } from '@mui/material';
  ...
  return isLoaded ? (
    <Box>
      ...
      >
        ...
        { !aniActive ? <div onClick={() => setAniActive(!aniActive)}>
          ...
          <PauseCircleIcon  sx={{fontSize:"40px",cursor:"pointer"}} />
          </div>
}
        </Box>
      </Popover>
    ...
    >
      ...
      return (
        ...
        <MapLine aniActive={aniActive} item={item} distance={distance} setDistance={setDistance} />
        {harshBreak && item?.data.map((innerItem, innerIndex) => {
...
{acceleration && item?.data.map((innerItem, innerIndex) => {
        return innerItem.packetType === filters.harshAcceleration ? (
        <Marker
          key={innerIndex}
          title={innerItem.packetType}
          position={{
...
{speed && item?.data.map((innerItem, innerIndex) => {
