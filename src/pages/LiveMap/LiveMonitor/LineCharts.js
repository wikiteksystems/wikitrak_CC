  ...
  const filledData = data.map((item) => {
    ...
    for (let i = 0; i < maxDataPoints; i++) {
      ...
      if (existingDataPoint) {
        ...
        // Fill in missing x-axis values with null or appropriate default value
        filledDataArray.push({ Time: i + 1, value: null });
      }
    }
    return { label: item.label, data: filledDataArray };
  });

  const chartWidth = Math.max(200, maxDataPoints * 80); // Minimum width of 800px
    ...
    <div style={{ width: '100%', height: '600px', overflowY: 'auto' , border: '1px solid #ccc'}}>
      <ResponsiveContainer width={chartWidth} height="100%" marginRight="10px">
        ...
        >
          ...
          <Brush dataKey="Date" height={30} stroke="#8884d8" tickFormatter={null} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
