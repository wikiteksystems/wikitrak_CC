...
const DetailMenu = ({
  ...
  const [vehicleGroupVisible, setVehicleGroupVisible] = useState(false);
  const [vehicleGroupDetailEditable, setVehicleGroupDetailEditable] =
    useState(false);
  const [vehicleGroupDetailVisible, setVehicleGroupDetailVisible] =
    useState(false);
  const [selectedVehicleGroup, setSelectedVehicleGroup] = useState(null);
  const [vehiclesInGroup, setVehiclesInGroup] = useState([]);
  ...
  const handleCheckboxClick = (e, selectType, selectedId) => {
    if (selectType === "select-all") {
      ...
      let findVehicleList = vehicleList.filter(
        (item) => item.vehicle_group === selectedId
      );
      let groupArray = [];
      if (e.target.checked) {
        let groupVehicleList = findVehicleList.map((list) => {
          let vehicleDetail = locationData.find(
            (imei) => imei.latestDocument.imei === list.imei[0].mac_id
          );
  ...
  const handleVehicleItemClick = (item) => {
    ...
    if (oem) dispatch(LiveMapActions.getVariantList(oem));
    if (model) {
      dispatch(LiveMapActions.getModelList(oem));
      dispatch(LiveMapActions.getSubModelList(model));
    }
    ...
    let list = [
      {
        type: "vehicles-in-group",
        label: (
          <div style={{ fontWeight: 500 }}> The vehicles in this group </div>
        ),
      },
    ];
    menuList.forEach((vehicle) => {
  ...
  const handleGroupColorChange = (color, type) => {
    setSelectedVehicleGroup({
      ...selectedVehicleGroup,
      [type]: color.toHexString().slice(1),
    });
  };
  const handleSelectChange = (value, type) => {
    if (type === "oem") {
  ...
  const handleClick = (type) => {
    if (type === "add-vehicle") {
      ...
      dispatch(
        LiveMapActions.saveVehicleGroup(
          newVehicleGroup,
          data,
          newVehicleGroup ? "" : selectedVehicleGroup.id
        )
      );
    } else if (type === "delete-vehicle-group") {
      dispatch(LiveMapActions.deleteVehicleGroup(selectedVehicleGroup.id));
                    ...
                    <>
                      ...
                      >
                        ...
                        <PlusCircleOutlined
                          ...
                          onClick={() => handleClick("add-vehicle")}
                        />
                      </div>
                      <div className="flex items-center m-1">
                        <Input
                              ...
                              item.type === "input" ? (
                                ...
                                >
                                  ...
                                  <span
                                    ...
                                    onClick={() => handleVehicleItemClick(item)}
                                  >
                                    {" "}
                                    {item?.registration_id}
                                  </span>
                  ...
                  {!menuCollapsed && vehicleDetailVisible && (
                    <>
                      ...
                      >
                        ...
                        />
                        <Header
                          title={selectedVehicle?.registration_id}
                          style={{ fontSize: 19, paddingInline: 0 }}
                        />
                        <div
                          className="h-full flex items-center cursor-pointer pr-2"
                          onClick={() =>
                            handleClick(
                              vehicleDetailEditable
                                ? "save-detail"
                      ...
                      <Menu
                        ...
                        items={[
                          ...(Vehi
                            ...
                            : LiveMapUtils.vehicleDetailMenuItems_addin),
                        ]
                          .filter(
                            (item) =>
                              item.type !== "input" || item.keyName !== "user"
                          ) // Exclude user detail inputs
                          .map((item, index) => ({
                            key: index,
                            label:
                              item.type === "input" ? (
                                item.keyName === "user" ? (
                                  ...
                                  <Input
                                    ...
                                    value={selectedVehicle[item.keyName]}
                                    onChange={(e) =>
                                      handleInputChange(e, item.keyName)
                                    }
                                  />
                                )
                              ) : item.type === "button" ? (
                                ...
                                <>
                                  <Select
                                    ...
                                    placeholder={item.label}
                                    value={
                                      selectedVehicle[item.keyName] === -1
                                        ? undefined
                                        : selectedVehicle[item.keyName]
                                    }
                                    onChange={(value) =>
                                      handleSelectChange(value, item.keyName)
                                    }
                                    dropdownRender={(menu) =>
                                      vehicleDetailEditable
                                        ? [
                                  ...
                                  {["variant", "model"].includes(
                                    ...
                                    <Radio
                                      ...
                                      checked={variantMode === item.keyName}
                                      onChange={(e) =>
                                        handleCheckboxClick(e, item.keyName)
                                      }
                                    />
                                  )}
                                </>
                ...
                <>
                  <>
                      ...
                      {!vehicleGroupDetailVisible ? (
                        <>
                          ...
                          <PlusCircleOutlined
                            ...
                            onClick={() => handleClick("add-vehicle-group")}
                          />
                        </>
                      ) : (
                        <>
                          <LeftCircleOutlined
                            className="px-2"
                            style={{ fontSize: 20, color: "white" }}
                            onClick={() => handleClick("vehicle_group_list")}
                          />
                          <Header
                            title={"Vehicle Group Detail"}
                            style={{ fontSize: 19, paddingInline: 0 }}
                          />
                          <div
                            className="h-full flex items-center cursor-pointer pr-2"
                    ...
                    <Menu
                      ...
                      selectable={false}
                      items={(vehicleGroupDetailVisible
                        ? [
                            ...LiveMapUtils.groupDetailMenuItems,
                            ...vehiclesInGroup,
                          ]
                        : selectedGroupList
                      ).map((item, index) => ({
                        ...
                        label:
                          item.type === "input" ? (
                            <Input
                              ...
                              value={selectedVehicleGroup[item.keyName]}
                              onChange={(e) =>
                                handleGroupInputChange(e, item.keyName)
                              }
                            />
                          ) : item.type === "button-delete" ? (
                            <Popconfirm
                              ...
                              disabled={newVehicleGroup}
                            >
                              <Button danger disabled={newVehicleGroup}>
                                {" "}
                                {item.label}{" "}
                              </Button>
                            </Popconfirm>
                            ...
                            <div className="flex justify-between">
                              {item.label}
                              <ColorPicker
                                disabled={!vehicleGroupDetailEditable}
                                value={selectedVehicleGroup[item.keyName]}
                                onChange={(e) =>
                                  handleGroupColorChange(e, item.keyName)
                                }
                              />
                            </div>
                          ) : item.type === "vehicles-in-group" ? (
                            ...
                            <div className="flex justify-between">
                              <span
                                ...
                                style={{ textOverflow: "ellipsis" }}
                                onClick={() =>
                                  handleVehicleGroupItemClick(item)
                                }
                              >
                                {item.group_name}
                              </span>
