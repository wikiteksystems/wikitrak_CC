...
function Map({
  ...
  // };
  const calculateAngleFromEast = (heading, imei) => {
    console.log(heading, 'heading')
    // Assuming heading is in degrees
    // Convert heading to radians
    const headingRadians = (heading * Math.PI) / 180;
    // Calculate the angle from the North direction clockwise
    // Adjust for the clockwise rotation by subtracting from 90 degrees
    const angleFromNorthClockwise = (Math.PI / 2 - headingRadians + 2 * Math.PI) % (2 * Math.PI);
    // Convert angle to degrees
    const angleDegrees = (angleFromNorthClockwise * 180) / Math.PI;
    // Return the angle in degrees
    console.log(angleDegrees,imei , "angleDegrees, imei")
    return angleDegrees;
};
            ...
            stopVehicleList.map((item, index) => (
              <Marker
                ...
                icon={{
                  ...
                  // rotation: (item?.latestDocument?.heading * 180) / Math.PI,
                  rotation: calculateAngleFromEast(
                    item?.latestDocument?.heading, item?.latestDocument?.imei
                  ),
                  scale: 1,
                }}
              >
                            ...
                            return (
                                <Marker
                                    ...
                                    icon={{
                                        ...
                                        fillOpacity: 2,
                                        strokeWeight: 1,
                                        rotation: calculateAngleFromEast(item?.latestDocument?.heading, item?.latestDocument?.imei),
                                        scale: 1,
                                    }}
                                >
                                    {selectedMarker === item.latestDocument._id && (
