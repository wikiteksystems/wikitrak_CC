...
const CreateParameter = ({ selectedParam, setSelectedParam }) => {
  ...
  return (
    <div>
       <h4 className="text-light my-4">Create Parameter</h4>
      {
        
        paramDetailMenuItems.map((item) => {
       ...
       <button className="btn btn-success p-1 " onClick={()=>{handleClick("save-detail")}} >Create</button>
     <div>
...
// const getComponent = (item,select)=>{
//   switch (item.type) {
//     case 'input':
//       return   <Input 
...
//   </Select>
//     case 'dropdown1':
//       return <div>Render UI for Option 3</div>;
//     case 'color':
//       return <div>Render UI for Option 4</div>;
//     default:
